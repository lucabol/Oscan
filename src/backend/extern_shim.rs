...
pub fn classify(
    ...
    for (_, ty) in params {
        match native_extern_type_kind(ty, program) {
            ...
            NativeExternTypeKind::Unsupported(reason) => {
                return Err(format!(
                    "extern function '{name}' parameter of type '{ty}' is not supported by the LLVM/Cranelift object backends ({reason})"
                ));
            }
    ...
    match native_extern_type_kind(return_type, program) {
        ...
        NativeExternTypeKind::Unsupported(reason) => {
            return Err(format!(
                "extern function '{name}' return type '{return_type}' is not supported by the LLVM/Cranelift object backends ({reason})"
            ));
        }
        NativeExternTypeKind::Simple => {}
    }
...
pub fn generate_source(
    ...
    out.push_str("/* Generated by Oscan object backend: user extern str ABI shims. */\n");
