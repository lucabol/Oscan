...
## Overview
The C-FFI implementation in Oscan is **complete and working correctly**. All features specified in section 9 of the spec are properly implemented.
...
- **Test**: `tests/positive/ffi.osc`, `tests/positive/ffi_advanced.osc`
...
- Regular Oscan functions receive arena as first parameter
- Correct C function signature mapping
- **Location**: `src/codegen.rs:850-860`
- Generated code example:
```c
const int32_t val = abs(osc_neg_i32(42));  // Direct call, no arena
```
...
2. **Pure functions cannot call extern functions** ✓
   ...
   - Test: `tests/negative/extern_in_pure.osc`
...
3. **Programmer responsibility for signature correctness** ✓
   ...
   - Type checking happens at Oscan level only
...
1. **ffi.osc**: Basic extern function (`abs` from stdlib)
   - Single extern block
   - Calling extern function with i32
   - **Result**: ✓ PASS

2. **ffi_advanced.osc**: Multiple extern functions (NEW)
...
| 9.4 Header includes | ✓ | stdint, stdio, stdlib, math, osc_runtime |
